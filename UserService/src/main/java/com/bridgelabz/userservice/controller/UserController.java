...
public class UserController {
    ...
    public ResponseEntity<ResponseDTO> userRegistration(@Valid @RequestBody RegisterUserDto registrationDTO , BindingResult bindingResult, HttpServletRequest request) throws MessagingException {
    ...
    public ResponseEntity resetPassword(@RequestParam(name = "password") String password,@RequestParam(value = "token",defaultValue = "") String urlToken){
        System.out.println(password +" "+urlToken);
...
//        List<User> allUsers = userService.getAllUsers(userField);
//        ResponseDTO responseDTO=new ResponseDTO("User List Fetched",200,allUsers);
//        return new ResponseEntity<>(responseDTO,HttpStatus.OK);
//    }

    @GetMapping()
    public Integer getUser(@RequestParam(name = "token") String token){
        System.out.println("token "+token);
        Integer user = userService.getUser(token);
//        ResponseDTO responseDTO =new ResponseDTO("Get User",200,user);
//        return new ResponseEntity(responseDTO,HttpStatus.OK);
        return user;
    }
